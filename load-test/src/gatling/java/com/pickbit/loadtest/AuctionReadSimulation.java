...
/**
 * 경매 목록/상세 조회 부하 테스트입니다.
 */
public class AuctionReadSimulation extends Simulation {
    ...
    private final ScenarioBuilder scenario = scenario("auction-read")
            .exec(http("GET /api/auctions")
                    .get("/api/auctions?page=0&size=20")
                    .check(status().is(200)))
            .exec(http("GET /api/auctions/{auctionId}")
                    .get("/api/auctions/" + LoadTestConfig.AUCTION_ID)
                    .check(status().is(200)))
            .exec(http("GET /api/auctions/products/{productId}")
                    .get("/api/auctions/products/" + LoadTestConfig.PRODUCT_ID)
                    .check(status().is(200)));
