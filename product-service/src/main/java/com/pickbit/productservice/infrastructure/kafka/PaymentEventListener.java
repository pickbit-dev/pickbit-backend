...
public class PaymentEventListener {
    ...
    public void handlePaymentMessage(
        ...
        try {
            switch (action) {
                case PaymentEventHandler.ESCROWED_ACTION -> eventHandler.handleEscrowed(eventId, aggregateId, messageBody, parseVersion(eventVersion));
                case PaymentEventHandler.SETTLED_ACTION -> eventHandler.handleSettled(eventId, aggregateId, messageBody, parseVersion(eventVersion));
                case PaymentEventHandler.REFUNDED_ACTION -> eventHandler.handleRefunded(eventId, aggregateId, messageBody, parseVersion(eventVersion));
                case PaymentEventHandler.FAILED_NO_PAYMENT_ACTION -> eventHandler.handleFailedNoPayment(eventId, aggregateId, messageBody, parseVersion(eventVersion));
                case PaymentEventHandler.CANCELLED_BEFORE_PAYMENT_ACTION -> eventHandler.handleCancelledBeforePayment(eventId, aggregateId, messageBody, parseVersion(eventVersion));
