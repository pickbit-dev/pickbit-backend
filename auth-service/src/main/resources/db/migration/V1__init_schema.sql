...
CREATE TABLE `auth_provider_link` (
  `account_id` bigint NOT NULL,
  `created_date` datetime(6) NOT NULL COMMENT '생성 일시',
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'id',
  `updated_date` datetime(6) NOT NULL COMMENT '업데이트 일시',
  `provider_id` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `provider` enum('GOOGLE','KAKAO','LOCAL','NAVER') COLLATE utf8mb4_unicode_ci NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_auth_provider_link_provider_id` (`provider`,`provider_id`),
  UNIQUE KEY `uk_auth_provider_link_account_provider` (`account_id`,`provider`),
  KEY `idx_auth_provider_link_account_id` (`account_id`),
  CONSTRAINT `fk_auth_provider_link_account` FOREIGN KEY (`account_id`) REFERENCES `auth_account` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
CREATE TABLE `inbox` (
