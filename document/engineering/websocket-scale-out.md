...
기본 100ms 로 시작했다. 사람 눈에 거의 무지각이고 동시에 폭주 트래픽을 30:1 수준으로 줄인다. deploy 는 `${AUCTION_WS_DEBOUNCE_MS:100}` 패턴으로 환경변수에서 조정할 수 있게 두었다. 마감 카운트다운 화면에서 1~2 frame 늦어 보인다는 피드백이 들어오면 50ms 로 내리면 된다.
